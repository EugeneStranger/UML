...
public:
	...
	void set_consumption_per_second(int speed)
	{
		...
		else if (speed > 250) consumption_per_second = DEFAULT_CONSUMPTION_PER_SECOND * 20;
	}
	void start()
	{
		is_started = true;
	...
	void info()const
	{
		cout << "Consumption:\t" << DEFAULT_CONSUMPTION << " liters per 100 km.\n";
	...
	void panel()const
	{
		while (driver_inside)
		{
			...
			for (int i = 0; i < speed/3; i++)
			{
				...
				SetConsoleTextAttribute(hConsole, 0xBC);
				cout << "|";
			}
			HANDLE hConsole = GetStdHandle(STD_OUTPUT_HANDLE);
